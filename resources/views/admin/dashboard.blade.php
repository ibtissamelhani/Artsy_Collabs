        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="flex gap-4 font-bold px-6 py-4 text-right">
                    <form method="POST" action="{{ route('projectUsers.updateStatus', ['projectUserId' => $request->id]) }}">
                        @csrf
                        @method('PUT') 
                        <button type="submit" class="font-medium  text-green-600 dark:text-green-500 hover:underline">Accept</button>
                    </form>
                    <form method="POST" action="{{ route('projectUsers.destroy', ['projectUserId' => $request->id]) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="font-medium text-red-600 dark:text-red-500 hover:underline">Refuse</button>
                    </form>
                </td>
            </tr>
            @endforeach
